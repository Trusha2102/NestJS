import { Controller, Post, Body, HttpException, HttpStatus, Get, Res, Param, SetMetadata, UseGuards, UseInterceptors } from '@nestjs/common';
...
async login(@Body() loginUserDto: LoginUserDto, @Res() res: Response): Promise<void> {
  try {
    ...
    const isPasswordValid = await bcrypt.compare(loginUserDto.password.trim(), user.password.trim());
